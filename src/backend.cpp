...
// Load theme handling with graceful fallback for non-omarchy systems
    loadTheme();
    // Watch theme files only if they exist (omarchy specific). This is a no-op on other systems.
    watchTheme();
...
// Reads GTK 3/4 CSS color definitions to extract the system palette.
// Priority: XDG portal accent-color > gtk-4.0/colors.css > gtk-3.0/gtk.css
static GtkColors detectGtkColors() {
    GtkColors result;

    // 1. Try XDG portal for accent-color (GNOME 47+, KDE 6.2+).
    //    Returns a (ddd) D-Bus struct: R, G, B in [0.0, 1.0].
    ...
    if (bus.isConnected()) {
        QDBusMessage request = QDBusMessage::createMethodCall(
            ...
            QStringLiteral("Read"));
        request << QStringLiteral("org.freedesktop.appearance") << QStringLiteral("accent-color");
            ...
            if (v.userType() == qMetaTypeId<QDBusArgument>()) {
                ...
                if (arg.currentType() == QDBusArgument::StructureType) {
                    ...
                    const QColor c = QColor::fromRgbF(
                        qBound(0.0, r, 1.0), qBound(0.0, g, 1.0), qBound(0.0, b, 1.0));
                    if (c.isValid())
                        result.accent = c.name();
                }
            }
        }
    }

    // 2. Parse GTK CSS files for background, foreground, and accent colors.
    //    Reads gtk-4.0/colors.css first, then gtk-3.0/gtk.css as fallback.
    const QString configDir = qEnvironmentVariable(
        "XDG_CONFIG_HOME", QDir::homePath() + QStringLiteral("/.config"));
    const QStringList gtkFiles = {
        QDir(configDir).filePath(QStringLiteral("gtk-4.0/colors.css")),
        QDir(configDir).filePath(QStringLiteral("gtk-4.0/gtk.css")),
        QDir(configDir).filePath(QStringLiteral("gtk-3.0/gtk.css")),
    };
    // Match: @define-color <name> #xxxxxx;
    static const QRegularExpression colorRe(
        QStringLiteral("@define-color\\s+(\\w+)\\s+(#[0-9A-Fa-f]{3,8})"));
    for (const QString &path : gtkFiles) {
        QFile f(path);
        if (!f.open(QIODevice::ReadOnly | QIODevice::Text))
            continue;
        QTextStream in(&f);
        while (!in.atEnd()) {
            const QRegularExpressionMatch m = colorRe.match(in.readLine());
            if (!m.hasMatch())
                continue;
            ...
            if (result.background.isEmpty()
                    && (name == QStringLiteral("window_bg_color")
                        || name == QStringLiteral("theme_bg_color")))
                result.background = color.name();
            // Foreground
            if (result.foreground.isEmpty()
                    && (name == QStringLiteral("window_fg_color")
                        || name == QStringLiteral("theme_fg_color")))
                result.foreground = color.name();
            // Accent (only if portal didn't give us one)
            if (result.accent.isEmpty()
                    && (name == QStringLiteral("accent_color")
                        || name == QStringLiteral("accent_bg_color")))
                result.accent = color.name();
        }
        // Stop at the first file that gave us something useful.
        if (!result.background.isEmpty() || !result.accent.isEmpty())
            break;
    }
    return result;
}

// Generic theme handling for non‑omarchy systems.
void Backend::loadTheme() {
    // Start with defaults based on current dark mode.
    m_themeBackground = m_darkMode ? QStringLiteral("#101010") : QStringLiteral("#ffffff");
    m_themeForeground = m_darkMode ? QStringLiteral("#eeeeee") : QStringLiteral("#222324");
    m_themeAccent     = m_darkMode ? QStringLiteral("#5584aa") : QStringLiteral("#2077b2");
    m_themeSelection  = m_darkMode ? QStringLiteral("#186a9a") : QStringLiteral("#2077b2");

    // Apply GTK system palette over the defaults (colors.toml wins last).
    const GtkColors gtk = detectGtkColors();
    if (!gtk.background.isEmpty())
        m_themeBackground = gtk.background;
    if (!gtk.foreground.isEmpty())
        m_themeForeground = gtk.foreground;
    if (!gtk.accent.isEmpty()) {
        m_themeAccent = gtk.accent;
        // Derive a selection highlight: blend accent 60% with background.
        const QColor accent(gtk.accent);
        const QColor bg(m_themeBackground);
        if (accent.isValid() && bg.isValid()) {
            m_themeSelection = QColor(
                qRound(accent.red()   * 0.6 + bg.red()   * 0.4),
                qRound(accent.green() * 0.6 + bg.green() * 0.4),
                qRound(accent.blue()  * 0.6 + bg.blue()  * 0.4)).name();
        }
    }

    // colors.toml overrides everything — user always wins.
    const QString configDir = qEnvironmentVariable(
        "XDG_CONFIG_HOME", QDir::homePath() + QStringLiteral("/.config"));
    const QString colorsPath = QDir(configDir).filePath("paperfry/theme/colors.toml");
    QString themeMode;
    QFile file(colorsPath);
    if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        QTextStream in(&file);
...
void Backend::watchTheme() {
    ...
    const QString themeDir = QDir(configDir).filePath("paperfry/theme");
    const QString colorsPath = QDir(themeDir).filePath("colors.toml");
    const QStringList watched = m_themeWatcher.files() + m_themeWatcher.directories();
    if (!watched.isEmpty())
        m_themeWatcher.removePaths(watched);
    if (QDir(themeDir).exists())
