...
class PaperfryTest : public QObject {
    Q_OBJECT
...
private slots:
    ...
    void loadsThemeFromConfig() {
        QTemporaryDir configDirectory;
        QVERIFY(configDirectory.isValid());

        const QByteArray originalConfig = qgetenv("XDG_CONFIG_HOME");
        ...
        QVERIFY(qputenv("XDG_CONFIG_HOME", configDirectory.path().toUtf8()));

        const QString themeDirectory = configDirectory.path()
            + QStringLiteral("/paperfry/theme");
...
QTEST_MAIN(PaperfryTest)
#include "tst_paperfry.moc"
